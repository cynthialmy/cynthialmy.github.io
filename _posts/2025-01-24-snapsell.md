...
The goal was to design a **controllable decision system** where AI handles text generation speed while humans retain authority over accuracy and appropriateness, stopping short of full end-to-end automation.
...
SnapSell is positioned for casual, mobile-first sellers who abandon listings due to text-writing friction. The main competition is the friction inside existing marketplaces and generic photo editors that still require manual copywriting, more than other AI listing apps.

The differentiator is the **photo-first, zero-form flow** combined with **transparent AI boundaries**: users review AI suggestions with clear limits on what the system can do. That positioning favors speed and trust over deep customization.
...
To mitigate this trust risk, I created **Snappy**, a cheerful otter mascot who anthropomorphizes the AI process. The choice was deliberate and functional:
...
3. **Create accountability:** Users associate output quality with a character instead of a faceless algorithm.
...
Test users mentioned Snappy by name in feedback: *"I feel like the otter just helped me sell something!"* Crucially, they attributed agency to Snappy, which made the model feel like a **collaborative assistant** rather than an autonomous black box and lifted copy/share rates.
...
The preview screen is mandatory: it is the core decision point where users validate AI output. This design embeds human judgment into the system:
...
This architecture treats the user as the **final validator** who must engage with the output. The system's goal is to earn that validation consistently.
...
| --------------------------- | ---------------------------------------------------------------------------------- |
| **Add Snappy the mascot**   | Builds trust in AI output; users attribute quality to a character, which feels safer than a faceless model. |
...
SnapSell's first version was only the starting point. The system evolved through three key inflection points as real usage revealed edge cases and economic realities.

### Iteration 1: Launch Week (Feb 2025)

**What broke:**
- Generation time averaged **12 seconds** versus the **7s** target because image uploads were unoptimized.
...
2. **Human-in-the-Loop is a first-class feature:** Forcing preview-before-copy introduced friction but **increased long-term trust**. Users who validated output once were more likely to return. Full automation would have optimized for first-time conversion but destroyed retention.
...
4. **Iteration Speed > Initial Accuracy:** Launching with 85% hallucination-free output (well short of 99%) allowed faster user feedback. Each iteration addressed real usage patterns rather than hypothetical edge cases.

5. **System Design Beats Feature Lists:** Candidates who show decision flows, failure modes, and trade-off matrices signal **systems thinking** beyond shipping isolated features.
...
Snappy, my illustrated otter mascot, became more than a character. It was a **trust mechanism** that bridged human creativity and AI utility. Users needed to believe the AI was working *with* them while they stayed in control of the listing.
